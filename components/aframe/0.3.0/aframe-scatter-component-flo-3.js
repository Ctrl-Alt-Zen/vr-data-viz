AFRAME.registerComponent('graph', {
  ...
  update: function () {
    ...
    var originPointPosition = (-width / 2) + ' '+ (height*2) +' ' + (depth/2);
    ...
    var xLabelPosition = (width / 3.2) + ' ' + '-3' + ' ' + '0';
    ...
    d3.select('#' + originPointID)
      ...
      .attr('bmfont-text', 'text: Dissolved O2 (mg/L); color: #FFF')

      .attr('position', xLabelPosition);



    var yLabelPosition = (width + 0.05) + ' ' + (-1.75) + ' ' + (-depth);
    ...
    d3.select('#' + originPointID)
      ...
      .attr('bmfont-text', 'text: Depth (m); color: #FFF')

      .attr('position', yLabelPosition);



    var zLabelPosition = (width + 0.02) + ' ' + '-2.75' + ' ' + (-depth / 2);
    ...
    d3.select('#' + originPointID)
      ...
      .attr('bmfont-text', 'text: Fluorescence; color: #FFF')
    ...
    if (data.csv) {
      ...
      var cScale = d3.scale.ordinal()
      										 ...
      										 .range(["hsl(250, 100%, 50%)","hsl(230, 100%, 50%)","hsl(210, 100%, 50%)", "hsl(190, 100%, 50%)", "hsl(170, 100%, 50%)","hsl(150, 100%, 50%)","hsl(130, 100%, 50%)","hsl(110, 100%, 50%)","hsl(90, 100%, 50%)","hsl(70, 100%, 50%)","hsl(50, 100%, 50%)","hsl(30, 100%, 50%)"]);
      ...
      var plotData = function (data) {
        ...
        var xExtent = d3.extent(data, function (d) { return d.DissolvedO2mgL; });
        ...
        var yExtent = d3.extent(data, function (d) { return d.DepthM; });
        ...
        var zExtent = d3.extent(data, function (d) { return d.Fluorescence; });
        ...
        originPoint.selectAll('a-sphere')
                   ...
                   .attr('radius', 0.03)  //CHANGE RADIUS HERE WITH FUNCTION(d)
                   ...
                   .attr('position', function (d) {
                     ...
                     return xScale(d.DissolvedO2mgL) + ' ' + yScale(d.DepthM) + ' ' + zScale(d.Fluorescence);
                     //THESE VALUES MUST AGREE WITH ABOVE DEFINITIONS
...
//  var texture = new THREE.TextureLoader().load('https://cdn.rawgit.com/bryik/aframe-scatter-component/master/assets/grid.png');
  ...
  var texture = new THREE.TextureLoader().load('../images/grid.png');
  ...
  var material = new THREE.MeshBasicMaterial({map: texture, side: THREE.DoubleSide, transparent: true, opacity: 0.2});
...
function labelMaker (dataEl, graphBoxWidth) {
  ...
  var depthM = 'Depth (m): ' + dataValues.DepthM + '\n \n';
